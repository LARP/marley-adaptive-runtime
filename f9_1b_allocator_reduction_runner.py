"""
f9_1b_allocator_reduction_runner.py
===================================
Phase F9-1B -- Confirmatory Allocator Policy Isolation Campaign.

Protocol (frozen, authoritative):
  docs/F9_1B_PREREGISTERED_ALLOCATOR_PROTOCOL_01.md
Governance:
  docs/private/F9_1B_DIRECTOR_AUTHORIZATION_LETTER_TO_TECH_DIRECTOR_01.md
  docs/private/F9_1B_CONSULTANT_OPINION_01.md
...
Intervention surface (F9-1B arm only; Workspaces UNTOUCHED / DEFAULT)
---------------------------------------------------------------------
  - PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True"
  - cuBLAS, cuDNN SDPA, benchmark, deterministic, TF32: UNTOUCHED / DEFAULT (identical to Control).
"""
...
# ---------------------------------------------------------------------------
PROTOCOL_REFERENCE = "docs/F9_1B_PREREGISTERED_ALLOCATOR_PROTOCOL_01.md"
...
# Frozen block order (V5/V6). Hash must match 323fda6381c065db0c7dc72d2d665b84ae7828cb58beca52d49e6c7574580411.
FROZEN_SEQUENCE_RAW = "I->C|C->I|C->I|I->C|I->C|I->C|I->C|C->I|C->I|C->I"
FROZEN_SEQUENCE_HASH = "323fda6381c065db0c7dc72d2d665b84ae7828cb58beca52d49e6c7574580411"
...
def condition_config(condition: str) -> Dict[str, Any]:
    ...
    if condition == "intervention":
        return {
            **common,
            "allocator_policy": "expandable_segments:True (Virtual Memory API)",
            "PYTORCH_CUDA_ALLOC_CONF": os.environ.get("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True"),
            "description": "isolated allocator policy (expandable segments)",
        }
    return {
        **common,
...
def apply_condition_config(condition: str) -> Dict[str, Any]:
    ...
    # Workspaces and math are completely standard/default in both arms
...
def run_worker(args: argparse.Namespace) -> None:
    ...
    print("=" * 80)
    print(f"  F9-1B RUN -- Block {args.block} | position {args.position} | condition {condition.upper()}")
    ...
    result: Dict[str, Any] = {
        ...
        "governance": ("Confirmatory paired campaign. Allocator policy isolated; "
                       "workspaces and SDPA kernels default/untouched in both arms."),
    }
    ...
    try:
        ...
        print(f"\n[F9-1B ABORT] Exception during execution: {error_occurred}")
...
def analyze(blocks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Criterion C: mechanistic (median dCat4 < 0 and upper CI < 0)
    crit_c = bool(boot_cat4 and boot_cat4["median"] < 0 and boot_cat4["ci95_high"] < 0)

    # Criterion D: anti-shifting (check if reduction in Cat4 shifted to Cat3)
    shifted = 0
    per_block_shift: List[Dict[str, Any]] = []
    for b, dc4, dc3 in zip(valid, d_cat4, d_cat3):
        if dc4 < 0:
            ck = max(0.0, dc3) / abs(min(dc4, 0.0))
        else:
            ck = None
        is_shift = bool(ck is not None and ck >= 0.5)
        shifted += int(is_shift)
        per_block_shift.append({"block": b["block"], "delta_cat4_mb": dc4,
                                "delta_cat3_mb": dc3, "C_k": ck, "shifted": is_shift})
...
def write_report(args: argparse.Namespace, telemetry: Dict[str, Any]) -> None:
    ...
    with open(args.report, "w", encoding="utf-8") as f:
        w = f.write
        w("# F9-1B — Confirmatory Allocator Policy Isolation — Report\n\n")
        w(f"**Date:** {telemetry['timestamp']}  \n")
        w(f"**Protocol:** [`{PROTOCOL_REFERENCE}`](F9_1B_PREREGISTERED_ALLOCATOR_PROTOCOL_01.md)  \n")
        ...
        w(f"- **C (mechanistic):** median(ΔCat4)<0 and upper CI<0 → "
...
def run_campaign(args: argparse.Namespace) -> None:
    ...
    print("=" * 80)
    print("  PHASE F9-1B -- CONFIRMATORY ALLOCATOR POLICY ISOLATION CAMPAIGN")
    print(f"  Protocol: {PROTOCOL_REFERENCE}")
    ...
    completed_block_nums = set()
    if os.path.exists(args.checkpoint):
        try:
            with open(args.checkpoint, "r", encoding="utf-8") as f:
                ckpt_data = json.load(f)
                if ckpt_data.get("sequence_hash") == FROZEN_SEQUENCE_HASH:
                    for b in ckpt_data.get("blocks", []):
                        if (b.get("control") and b.get("intervention")
                                and b["control"].get("error") is None
                                and b["intervention"].get("error") is None):
                            blocks.append(b)
                            completed_block_nums.add(b["block"])
            if completed_block_nums:
                print(f"[Coordinator] Resuming from checkpoint: {len(completed_block_nums)} blocks already completed: {sorted(completed_block_nums)}")
        except Exception as e:
            print(f"[Coordinator WARNING] Could not parse checkpoint ({e}); starting clean.")
            blocks = []

    for block_idx, (first, second) in enumerate(sequence, start=1):
        if block_idx in completed_block_nums:
            print(f"\n[Coordinator] Skipping BLOCK {block_idx}/10 (already completed in checkpoint).")
            continue
        ...
        for position, condition in ((1, first), (2, second)):
            ...
            run_out = os.path.join(args.run_output_dir, f"f9_1b_{run_label}.json")
            env = os.environ.copy()
    ...
    telemetry: Dict[str, Any] = {
        "timestamp": datetime.datetime.now().isoformat(),
        "phase": "F9-1B (Confirmatory Allocator Policy Isolation)",
        "protocol_reference": PROTOCOL_REFERENCE,
    ...
    print("=" * 80)
    print(f"  F9-1B CAUSAL SUCCESS: {analysis['causal_success']} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Marley Phase F9-1B -- Confirmatory Allocator Policy Isolation")
    ...
    p.add_argument("--run-out", type=str, default="logs/f9_1b_run.json")
    p.add_argument("--run-output-dir", type=str, default="logs/f9_1b")
    p.add_argument("--checkpoint", type=str, default="logs/f9_1b_checkpoint.json")
    p.add_argument("--output", type=str, default="logs/f9_1b_telemetry.json")
    p.add_argument("--report", type=str, default="docs/F9_1B_ALLOCATOR_REDUCTION_REPORT_01.md")
    return p.parse_args()
