"""
f7_d4_multistep_validation.py
==============================
Phase F7-D4 (reduced multi-step) -- Validation of the F7-D3 cond->uncond seam
release across several denoising steps + VAE.

Workload: 1280x720, 33 frames, N adaptive steps (default 10), seed 42.
Intervention (single, unchanged from F7-D3): empty_cache at every cond->uncond
seam within each step. No other optimization. Runtime untouched (subclass seam).

Goal (memory question):
  Does Reserved/NVML stay <= ~4,800 MB across multiple steps WITHOUT progressive
  re-accumulation, and does the VAE decode complete afterward?

Note: cadence gate for 720p is "no catastrophic regression vs ~60 s/step",
NOT the 480p/F6 E2E <600 s objective (unachievable at this cadence).

Evidence: MEASURED / OBSERVED / DERIVED / HYPOTHESIS.
Governance: runner + spec only. Does NOT authorize runtime integration.
"""
...
SAFE_ABORT_NVML_MB = 5050.0     # near the 4.8GB gate but protects against runaway to 6GB
CANONICAL_PROMPT = (
    ...
    def run_validation(self, prompt, negative_prompt, height, width, num_frames,
        ...
        try:
            for step_idx, t in enumerate(timesteps):
                ...
                # F7-D4b variant: drain allocator carryover at step start (before cond)
                if release_strategy == "seam_and_step_start":
                    gc.collect()
        ...
        out["release_strategy"] = release_strategy
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Marley Phase F7-D4 -- reduced multi-step seam-release validation")
    ...
    p.add_argument("--release-strategy", type=str, default="seam",
                   choices=["seam", "seam_and_step_start"],
                   help="seam = empty_cache at cond->uncond (F7-D3/D4); seam_and_step_start = also at step start")
    p.add_argument("--output", type=str, default="logs/f7_d4_multistep_validation_telemetry.json")
    p.add_argument("--report", type=str, default="docs/F7_D4_MULTISTEP_VALIDATION_REPORT_01.md")
...
def main() -> None:
    ...
    out_mp4 = None if args.no_vae else f"logs/f7_d4_{args.width}x{args.height}_{args.frames}f_{args.steps}st_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"

    print("=" * 80)
    print("  MARLEY RUNTIME -- PHASE F7-D4 (REDUCED): MULTI-STEP SEAM-RELEASE VALIDATION")
    print("  (Intervention = empty_cache at every cond->uncond seam, from F7-D3)")
    print("=" * 80)
    ...
    print(f"  Hard Gate: <= {HARD_GATE_VRAM_MB:.0f} MB | Safe Abort: > {SAFE_ABORT_NVML_MB:.0f} MB")
    print(f"  Release strategy: {args.release_strategy} (seam = cond->uncond; seam_and_step_start adds step-start drain)")
    print(f"  D3 single-step reference: peak NVML {D3_PEAK_NVML_MB} MB, Reserved {D3_PEAK_RESERVED_MB} MB")

    if args.dry_run:
        print("\n[DRY-RUN] Parameter/environment verification. NO inference.")
        print(f"  {args.steps} denoise steps, each with cond->uncond empty_cache; VAE={'on' if not args.no_vae else 'off'}")
    ...
    try:
        pipeline = MultistepSeamReleasePipeline(
            device=args.device, dit_dtype=torch.float16,
            vae_dtype=torch.bfloat16, text_dtype=torch.bfloat16)
        print(f"\n>> Executing F7-D4 ({args.steps} steps @ {args.width}x{args.height}, {args.frames}f)...")
        ...
        print(f"\n[F7-D4 ABORT] Exception during execution: {error_occurred}")
    finally:
    ...
    if len(step_end_res) >= 2:
        accumulation = round(step_end_res[-1] - step_end_res[0], 1)  # last step vs first step end-reserved
    ...
    gate_pass = peak_nvml <= HARD_GATE_VRAM_MB and not nan_inf and (vae_ok is not False)
    if gate_pass:
        result = "FAVORABLE (multi-step peak <=4800, VAE ok)" if vae_ok else "DiT-ONLY: peak <=4800 (VAE skipped)"
    else:
        result = "PARTIAL/NEGATIVE"

    print("\n" + "=" * 80)
    print("  PHASE F7-D4 (REDUCED): SCORECARD")
    print("=" * 80)
    print(f"  Peak NVML fisico        [OBSERVED] : {peak_nvml:7.1f} MB (D3 single-step: {D3_PEAK_NVML_MB:.1f})")
    print(f"  Peak Reserved           [OBSERVED] : {peak_reserved:7.1f} MB (D3: {D3_PEAK_RESERVED_MB:.1f})")
    ...
    telemetry: Dict[str, Any] = {
        ...
        "phase": "F7-D4 (reduced multi-step seam-release validation)",
        ...
        "intervention": f"empty_cache release strategy = {args.release_strategy} (from F7-D3 seam, F7-D4b adds step-start)",
        "release_strategy": args.release_strategy,
    ...
    with open(args.report, "w", encoding="utf-8") as f:
        w = f.write
        w("# F7-D4 (Reduced) Multi-Step Seam-Release Validation -- Report\n\n")
        ...
        w("**Intervention:** empty_cache at every cond->uncond seam (single variable, from F7-D3).\n\n")
        w("---\n\n## 1. Result\n\n")
        w(f"- Peak NVML: {peak_nvml:.1f} MB (D3 single-step: {D3_PEAK_NVML_MB:.1f})\n")
        w(f"- Peak Reserved: {peak_reserved:.1f} MB\n")
        w(f"- Reserved accumulation (last-first step-end): {accumulation} MB\n")
        if probe:
            w(f"- Denoise: {probe.get('denoise_time_s')} s; cadence {probe.get('cadence_avg_s')} s/step; seam releases {probe.get('seam_release_count')}\n")
        if probe and "vae" in probe:
            v = probe["vae"]
        ...
        for i, r in enumerate(step_end_res):
            w(f"- step {i+1}: {r}\n")
        w("\n## 3. Governance\n\n")
        w("Reduced multi-step causal validation in an isolated runner. Does NOT authorize "
          "runtime integration; full 30-step / E2E remain pending.\n")
